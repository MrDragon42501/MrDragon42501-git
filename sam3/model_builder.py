...
def build_tracker(
    ...
    if with_backbone:
        if visual_backbone is None:
            visual_backbone = _create_vision_backbone(compile_mode=compile_mode)
        backbone = SAM3VLBackbone(scalp=1, visual=visual_backbone, text=None)
...
def build_sam3_image_model(
    ...
    if checkpoint_path is not None:
        ...
        # 既不指定路径也不从HF加载，提示未加载权重
        print("Warning: No checkpoint loaded. Model will be initialized randomly.")
...
def build_sam3_video_model(
    checkpoint_path: Optional[str] = r"E:\VS code\SAM3\sam3.pt",
    load_from_HF=True,
    ...
    # Build Detector components (visual neck shared with tracker)
    visual_neck = _create_vision_backbone()
    text_encoder = _create_text_encoder(bpe_path)
    backbone = SAM3VLBackbone(scalp=1, visual=visual_neck, text=text_encoder)
    transformer = _create_sam3_transformer(has_presence_token=has_presence_token)

    # Build Tracker module (reuses the same visual neck for memory efficiency)
    tracker = build_tracker(
        apply_temporal_disambiguation=apply_temporal_disambiguation,
        with_backbone=True,
        visual_backbone=visual_neck,
    )
    ...
    if checkpoint_path is not None:
        ...
        # The tracker backbone reuses the detector's visual neck. If the checkpoint
        # was created for a model whose tracker had no backbone (the original default),
        # duplicate the detector visual backbone keys so strict loading succeeds.
        if not any(k.startswith("tracker.backbone.") for k in ckpt):
            tracker_visual_keys = {
                k.replace("detector.backbone.vision_backbone.", "tracker.backbone.vision_backbone."): v
                for k, v in ckpt.items()
                if k.startswith("detector.backbone.vision_backbone.")
            }
            ckpt.update(tracker_visual_keys)
