...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def add_prompts(
        ...
        """
        Add text, point or box prompts on multiple frames in a single call.

        Each prompt in `prompts` is a dict with keys:
            - frame_idx (int): frame index to add the prompt on.
            - text_str (Optional[str]): shared text prompt (applied to all frames).
            - boxes_xywh (Optional[List[List[float]]]): box prompts in [xmin, ymin, w, h].
            - box_labels (Optional[List[int]]): labels for the box prompts.

        Returns:
            dict: mapping from frame_idx to inference outputs on that frame.
        """
        num_frames = inference_state["num_frames"]
        assert len(prompts) > 0, "at least one prompt must be provided"

        # since it's a semantic prompt, we start over once
        self.reset_state(inference_state)

        # use text from the first prompt that provides it (shared across all frames)
        text_str = None
        for p in prompts:
            if p.get("text_str") is not None:
                text_str = p["text_str"]
                break
        ...
        # 2) handle each frame's box prompt and run inference to seed tracker states
        outputs_per_frame = {}
        for p in prompts:
            frame_idx = p["frame_idx"]
            boxes_xywh = p.get("boxes_xywh")
            box_labels = p.get("box_labels")
            ...
            outputs_per_frame[frame_idx] = self._postprocess_output(inference_state, out)

        return outputs_per_frame

    @torch.inference_mode()
    def init_video_from_single_frame_seeds(self, inference_state, seeds):
        """
        Initialize tracker masklets from external single-frame segmentation masks.
        ...
        """
        assert len(seeds) > 0, "at least one seed must be provided"
        num_frames = inference_state["num_frames"]
        input_batch = inference_state["input_batch"]
        feature_cache = inference_state["feature_cache"]

        # 1) reset and initialize metadata
        self.reset_state(inference_state)
        inference_state["tracker_metadata"] = self._initialize_metadata()

        # 2) precompute backbone features for seed frames
        for seed in seeds:
            frame_idx = seed["frame_idx"]
            assert 0 <= frame_idx < num_frames, (
                f"{frame_idx=} out of range for {num_frames} frames"
            )
            img = input_batch.img_batch[frame_idx].unsqueeze(0)
            backbone_out = self.tracker.forward_image(img)
            feature_cache[frame_idx] = (
                input_batch.img_batch[frame_idx],
                {"tracker_backbone_out": backbone_out},
            )

        # 3) create tracker state for object 0 using the first seed frame
        first_seed = seeds[0]
        first_mask = torch.from_numpy(first_seed["mask"]).float().unsqueeze(0).to(self.device)
        tracker_states_local = inference_state["tracker_inference_states"]
        self._tracker_add_new_objects(
            frame_idx=first_seed["frame_idx"],
            num_frames=num_frames,
            new_obj_ids=[0],
            new_obj_masks=first_mask,
            tracker_states_local=tracker_states_local,
            orig_vid_height=inference_state["orig_height"],
            orig_vid_width=inference_state["orig_width"],
            feature_cache=feature_cache,
        )
        tracker_state = tracker_states_local[0]

        # 4) add masks for the remaining seed frames on the same object
        for seed in seeds[1:]:
            mask = torch.from_numpy(seed["mask"]).float().to(self.device)
        ...
        # 5) update tracker metadata for object 0
        tracker_metadata = inference_state["tracker_metadata"]
        tracker_metadata["obj_ids_per_gpu"][self.rank] = np.array([0], dtype=np.int64)
        tracker_metadata["obj_ids_all_gpu"] = np.array([0], dtype=np.int64)
        tracker_metadata["num_obj_per_gpu"][self.rank] = 1
        tracker_metadata["max_obj_id"] = 0
        tracker_metadata["obj_id_to_score"] = {0: float(first_seed.get("score", 1.0))}
        if self.rank == 0:
            tracker_metadata["rank0_metadata"]["obj_first_frame_idx"][0] = first_seed["frame_idx"]

        # 6) mark seed frames as having outputs
        for seed in seeds:
            inference_state["previous_stages_out"][seed["frame_idx"]] = "_THIS_FRAME_HAS_OUTPUTS_"

    @torch.inference_mode()
    def propagate_in_video_tracker_only(
        self,
        inference_state,
        start_frame_idx=None,
        max_frame_num_to_track=None,
        ...
        """
        Tracker-only propagation (no detector) after initialization from single-frame seeds.
        """
        ...
        for frame_idx in tqdm(processing_order, desc=f"tracker_propagation (reverse={reverse})"):
            # ensure backbone features are cached for this frame
            ...
            # run tracker propagation for one frame
            ...
            if self.rank == 0:
                # build video-resolution masks dict
                obj_id_to_mask = {}
                for obj_id, low_res_mask in zip(obj_ids_local, low_res_masks_local):
                    video_res_mask = self._convert_low_res_mask_to_video_res(
                        low_res_mask, inference_state
                    )
                    obj_id_to_mask[int(obj_id)] = video_res_mask
                ...
                obj_id_to_tracker_score = {
                    int(obj_id): tracker_scores_local[i].item()
                ...
                yield frame_idx, self._postprocess_output(inference_state, out)
            else:
