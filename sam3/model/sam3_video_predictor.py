...
class Sam3VideoPredictor:
    ...
    def handle_request(self, request):
        ...
        elif request_type == "add_prompts":
            return self.add_prompts(
                session_id=request["session_id"],
                prompts=request["prompts"],
            )
        elif request_type == "init_video_from_single_frame_seeds":
            return self.init_video_from_single_frame_seeds(
                session_id=request["session_id"],
                seeds=request["seeds"],
            )
    ...
    def handle_stream_request(self, request):
        ...
        elif request_type == "propagate_in_video_tracker_only":
            yield from self.propagate_in_video_tracker_only(
                session_id=request["session_id"],
                propagation_direction=request.get("propagation_direction", "both"),
                start_frame_idx=request.get("start_frame_index", None),
                max_frame_num_to_track=request.get("max_frame_num_to_track", None),
    ...
    def add_prompts(
        ...
        """Add multiple text, box and/or point prompts on multiple video frames."""
        logger.debug(
            f"add prompts in session {session_id}: {prompts}"
        )
        session = self._get_session(session_id)
        inference_state = session["state"]

        outputs_per_frame = self.model.add_prompts(
            inference_state=inference_state,
            prompts=prompts,
        )
        return {"outputs_per_frame": outputs_per_frame}

    def init_video_from_single_frame_seeds(
        self,
        session_id: str,
        seeds: List[dict],
    ):
        """Use masks/bboxes/scores from single-frame segmentation to initialize tracker masklets.

        Args:
            seeds: list of dicts, each with keys:
                - frame_idx (int)
                - mask (np.ndarray, HxW bool)
                - bbox (list/tuple [x1, y1, x2, y2], optional)
                - score (float)
        """
        logger.debug(
            f"init video from single-frame seeds in session {session_id}: {len(seeds)} seeds"
        )
        session = self._get_session(session_id)
        inference_state = session["state"]
        self.model.init_video_from_single_frame_seeds(
            inference_state=inference_state,
            seeds=seeds,
        )
    ...
    def propagate_in_video_tracker_only(
        ...
        """Propagate using tracker only (no detector), initialized from single-frame seeds."""
        logger.debug(
            f"tracker-only propagation in session {session_id}: "
        ...
        try:
            ...
            if propagation_direction in ["both", "forward"]:
                for frame_idx, outputs in self.model.propagate_in_video_tracker_only(
                    inference_state=inference_state,
            ...
            logger.debug(
                f"tracker-only propagation ended in session {session_id}; {self._get_session_stats()}"
            )
