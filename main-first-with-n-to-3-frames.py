...
sys.path.append(r"E:\VS code\SAM3")
...
def get_box_prompts_from_frames(frames, image_model, frame_indices):
    """对指定帧索引列表分别调用单帧水位线识别，返回各帧像素Y坐标、框提示、mask及score"""
    waterline_ys = []
    box_prompts = []
    masks = []
    bboxes = []
...
def filter_n_to_best_3(frame_indices, waterline_ys, box_prompts, masks, bboxes, scores, iqr_factor=1.5):
    """
    用 IQR 四分位法剔除水位线异常帧，再从剩余帧中择优选 3 帧（时序覆盖：最早/中间/最晚）。
    返回 (best3_fi, best3_waterline_ys, best3_box_prompts, best3_masks, best3_bboxes, best3_scores)。
    """
    n = len(frame_indices)
    if n <= 3:
        return frame_indices, waterline_ys, box_prompts, masks, bboxes, scores

    Y = np.array(waterline_ys, dtype=float)
    ...
    print("\n" + "=" * 62)
    print(f"📈 [n→3 筛选] IQR 四分位法：共 {n} 帧，目标保留 3 帧")
    print(f"     Q1={q1:.1f}  Q3={q3:.1f}  IQR={iqr:.1f}")
    print(f"     正常区间 = [{lower:.1f}, {upper:.1f}]  (k={iqr_factor})")
    print(f"     {'帧号':>6} | {'水位Y':>8} | 判定")
    print(f"     {'-' * 6}-+-{'-' * 8}-+--------------------")
    for i in range(n):
        if keep_mask[i]:
            verdict = "保留"
        elif Y[i] > upper:
            verdict = f"剔除(超上限 {Y[i] - upper:.1f}px)"
        else:
            verdict = f"剔除(低下限 {lower - Y[i]:.1f}px)"
        print(f"     {frame_indices[i]:>6} | {Y[i]:>8.1f} | {verdict}")

    # 兜底：若剔除后不足 3 帧，按“离正常区间最近”的顺序从异常帧中补回
    n_outliers = len(outlier_indices)
    if len(keep_indices) < 3:
        def dist_to_bounds(i):
            if Y[i] > upper:
                return Y[i] - upper
            if Y[i] < lower:
                return lower - Y[i]
            return 0.0
        outlier_indices.sort(key=dist_to_bounds)
        need = 3 - len(keep_indices)
        keep_indices = sorted(keep_indices + outlier_indices[:need])
        print(f"     ⚠️ IQR 剔除后不足 3 帧，已按离区间最近补回 {need} 帧")

    # ---- 第二步：剩余择优（时序覆盖：最早/中间/最晚）----
    if len(keep_indices) > 3:
        pick_positions = [0, len(keep_indices) // 2, len(keep_indices) - 1]
        pick_indices = sorted({keep_indices[p] for p in pick_positions})
    else:
        pick_indices = keep_indices

    best3_fi = [frame_indices[i] for i in pick_indices]
    best3_wy = [waterline_ys[i] for i in pick_indices]
    best3_bp = [box_prompts[i] for i in pick_indices]
    best3_masks = [masks[i] for i in pick_indices]
    best3_bboxes = [bboxes[i] for i in pick_indices]
    best3_scores = [scores[i] for i in pick_indices]

    print(f"\n  ✅ IQR 剔除 {n_outliers} 个异常帧，剩余 {len(keep_indices)} 帧 → 择优保留 {len(best3_fi)} 帧: {best3_fi}")
    print("=" * 62 + "\n")

    return best3_fi, best3_wy, best3_bp, best3_masks, best3_bboxes, best3_scores


def log_gpu_memory(tag=""):
    """打印当前 GPU 显存占用（仅在 CUDA 可用时）。"""
    if not torch.cuda.is_available():
        return
    allocated = torch.cuda.memory_allocated() // 1024**2
    reserved = torch.cuda.memory_reserved() // 1024**2
    prefix = f"[{tag}] " if tag else ""
    print(f"  {prefix}GPU 显存: allocated={allocated} MiB, reserved={reserved} MiB")


def process_single_video(v_path, out_path, v_name, checkpoint_path, save_target_folder, n_sample_frames=5):
    """
    处理单个视频并确保处理完成后完全释放模型与显存。
    n_sample_frames: 等间距抽取的帧数，经多项式拟合剔除异常后保留最优 3 帧。
    ...
    n_actual = min(n_sample_frames, len(frames))
    sample_indices = sorted(set(np.linspace(0, last_idx, n_actual, dtype=int).tolist()))
    print(f"  - 视频共 {len(frames)} 帧，等间距抽取 {len(sample_indices)} 帧: {sample_indices}")

    image_model = None
    video_predictor = None
    session_id = None

    # 提前声明，方便 finally 统一清理
    waterline_ys = []
    box_prompts = []
    masks = []
    bboxes = []
    ...
    try:
        # 2. 初始化图像模型，对关键帧分别做单帧水位线识别
        print("  正在加载 SAM3 Image 模型进行单帧识别...")
        log_gpu_memory("加载 Image 模型前")
        image_model = build_sam3_image_model(checkpoint_path=checkpoint_path)
        log_gpu_memory("加载 Image 模型后")

        frame_indices, waterline_ys, box_prompts, masks, bboxes, single_frame_scores = get_box_prompts_from_frames(
            frames, image_model, sample_indices
        )
        print(f"  - 已获取各帧水位线 Y 坐标: {waterline_ys}")

        # n帧 → 3帧 筛选：用多项式拟合剔除异常帧，保留最准的3帧
        frame_indices, waterline_ys, box_prompts, masks, bboxes, single_frame_scores = filter_n_to_best_3(
            frame_indices, waterline_ys, box_prompts, masks, bboxes, single_frame_scores
        )
        ...
        target_save_dir = save_target_folder
        ...
        try:
            ...
            for fi, mask, bbox in zip(frame_indices, masks, bboxes):
                ...
                if num_targets > 0:
                    ...
                    save_per_target_overlays(frames[fi], fi, seed_outputs, v_name, target_save_dir)
            ...
            # 失败后彻底清理旧 session 和碎片化显存，再开新 session
            try:
        ...
        if tracker_only_failed:
            ...
            for fi, box in zip(frame_indices, box_prompts):
                ...
                save_per_target_overlays(frames[fi], fi, outputs, v_name, target_save_dir)
        ...
        # 7. 完整清理：关闭会话、释放模型、删除局部大对象、强制 GC、回收 CUDA 缓存
        ...
        # 兜底清理 SAM3 类级 _ALL_INFERENCE_STATES（确保 session 不移除干净也不影响下一个视频）
        if video_predictor is not None:
            try:
                video_predictor._ALL_INFERENCE_STATES.clear()
            except Exception:
                pass

        # 显式将模型移回 CPU 以强制释放 CUDA 显存（比 del 更可靠，绕过 GC 延迟）
        if video_predictor is not None:
            try:
        ...
        print("  正在完整释放资源（模型、帧缓存、显存）...")
        log_gpu_memory("清理前")

        # 删除所有可能占用显存/内存的局部对象
        try:
            del video_predictor, image_model, frames, outputs_per_frame
            del result_frames, masks, bboxes, box_prompts, waterline_ys
            del single_frame_scores
        except NameError:
            pass
        try:
            del seeds, prompts, add_prompts_response, fallback_output
        except NameError:
            pass
...
def _video_worker(v_path, out_path, v_name, checkpoint_path, save_target_folder, n_sample_frames, result_queue):
    """子进程入口：在独立进程中处理单个视频，进程退出后由 OS 彻底回收全部显存。"""
    try:
        success = process_single_video(
            v_path, out_path, v_name, checkpoint_path, save_target_folder,
...
def batch_process_videos_auto(input_dir, output_dir, checkpoint_path, save_target_folder, n_sample_frames=5):
    """
    视频批量自动化分割流水线（子进程隔离模式）。
    每个视频在独立子进程中处理，进程退出后由操作系统强制回收全部 GPU 显存，
    确保下一个视频开始时显存占用与第一个视频完全相同（≈0）。
    """
    import multiprocessing as mp

    print("\n" + "=" * 60)
    print("🚀 启动视频自动化分割流水线 (子进程隔离模式)")
    print(f"   等间距抽取 {n_sample_frames} 帧，多项式拟合筛选最优3帧")
    print("   每个视频在独立子进程中运行，退出后显存由 OS 彻底回收")
    print("=" * 60)
    log_gpu_memory("主进程启动时")
    ...
    for idx, v_name in enumerate(video_files, 1):
        ...
        log_gpu_memory(f"视频 {v_name} 处理前(主进程)")

        ctx = mp.get_context("spawn")
        result_queue = ctx.Queue()
        p = ctx.Process(
            target=_video_worker,
            args=(v_path, out_path, v_name, checkpoint_path, save_target_folder,
                  n_sample_frames, result_queue)
        )
        p.start()
        p.join()  # 阻塞等待子进程结束；子进程退出后其显存被 OS 回收
        ...
        if p.exitcode == 0:
            try:
                ...
                success = False
            if not success:
                print(f"  ⚠️ 视频 {v_name} 处理未成功")
        else:
            print(f"  ⚠️ 子进程异常退出 (exitcode={p.exitcode})")

        p.close()
        log_gpu_memory(f"视频 {v_name} 处理后(主进程)")

    print("\n🎉 所有视频处理完毕！")
    log_gpu_memory("全部结束时(主进程)")
...
def main():
    ...
    paths =  {
        ...
        "input_video_folder": r"C:\Users\ShenYuLong\Downloads\HnVideoEditor_2026_08_12_145428821",
        "output_video_folder": r"E:\VS code\data\water_video_ROI\video_segment_with_problem\test",
        "save_target_folder": r"E:\VS code\SAM3\recognized_targets"
    }
    ...
    Path(paths["save_target_folder"]).mkdir(parents=True, exist_ok=True)
    batch_process_videos_auto(
        input_dir=paths["input_video_folder"],
        output_dir=paths["output_video_folder"],
        checkpoint_path=checkpoint_path,
        save_target_folder=paths["save_target_folder"],
        n_sample_frames=7
    )
