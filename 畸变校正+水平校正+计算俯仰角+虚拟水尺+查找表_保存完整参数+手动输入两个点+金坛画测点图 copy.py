...
def draw_virtual_gauge_template_transparent(
    lookup_df: pd.DataFrame,
    roi: Tuple[int, int, int, int],
    ...
    """
    生成透明背景的虚拟水尺PNG模板。
    仅绘制水尺刻度、标注和可选ROI框，不绘制原图背景。
    """
    h_img, w_img = canvas_size

    # 4通道透明画布：BGRA
    out = np.zeros((h_img, w_img, 4), dtype=np.uint8)
    ...
    if draw_roi_box:
        cv2.rectangle(out, (x, y), (x + w - 1, y + h - 1), (255, 255, 0, 255), 2)
...
def main():
   ...
   ###====新增透明背景水尺模板输出====###
    ...
    gauge_template_transparent = draw_virtual_gauge_template_transparent(
        lookup_df=lookup_df,
        roi=roi,
        canvas_size=(h_rot, w_rot),
        z_min=gauge_params.gauge_z_min,
        z_max=gauge_params.gauge_z_max,
        step_main=0.5,
        color=(0, 0, 255, 255),
    ...
    template_png_path = os.path.join(output_dir, "virtual_gauge_template_transparent.png")
    success, buf = cv2.imencode(".png", gauge_template_transparent)
    buf.tofile(template_png_path)
    print(f"[输出]透明背景虚拟水尺模板已保存：{template_png_path}")
    ###====新增结束====###
