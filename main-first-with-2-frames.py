...
def get_box_prompts_from_frames(frames, image_model):
    """针对首帧、尾帧分别调用单帧水位线识别，返回各帧像素Y坐标、框提示、mask及score"""
    frame_indices = sorted({0, len(frames) - 1})
    waterline_ys = []
    box_prompts = []
    masks = []
    bboxes = []
...
def process_single_video(v_path, out_path, v_name, checkpoint_path, save_target_folder, num_key_frames=2):
    """
    处理单个视频并确保处理完成后完全释放模型与显存。
    num_key_frames: 2 表示首帧+尾帧；3 表示首帧+中间帧+尾帧。
    ...
    if num_key_frames == 3:
        ...
        frame_indices = sorted({0, last_idx})
        print(f"  - 视频共 {len(frames)} 帧，提取首帧({frame_indices[0]})、尾帧({last_idx})进行图像分析...")
...
def _video_worker(v_path, out_path, v_name, checkpoint_path, save_target_folder, result_queue):
...
def batch_process_videos_auto(input_dir, output_dir, checkpoint_path, save_target_folder):
    """
    ...
    for idx, v_name in enumerate(video_files, 1):
        ...
        p = ctx.Process(
            ...
            args=(v_path, out_path, v_name, checkpoint_path, save_target_folder, result_queue)
        )
        p.start()
...
def main():
    ...
    paths =  {
        ...
        "input_video_folder": r"E:\VS code\data\water_video_ROI\original_video",#"E:\VS code\data\water_video_ROI\video_segment_with_problem"
